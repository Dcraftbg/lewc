...
bool build_ast_data(Qbe* qbe, AST* ast) {
    switch(ast->kind) {
    case AST_INT:
        nprintf("%s %lu", type_to_qbe(qbe->arena, ast->type), ast->as.integer.value);
        break;
    case AST_STRUCT_LITERAL: {
        // FIXME: assumes all the fields are in order
        StructLiteral* lit = &ast->as.struc_literal;
        for(size_t i = 0; i < lit->fields.len; ++i) {
            if(i > 0) nprintf(", ");
            if(!build_ast_data(qbe, lit->fields.items[i].value)) return false;
        }
    } break;
    default:
        unreachable("ast->kind=%d", ast->kind);
...
bool build_qbe_qbe(Qbe* qbe) {
    ...
    for(size_t i = 0; i < qbe->module->symbols.len; ++i) {
        ...
        if(s->kind == SYMBOL_GLOBAL) {
            AST* ast = s->ast;
            nprintf("data $%s = { ", name->data);
            if(!build_ast_data(qbe, ast)) return false;
            nprintfln(" }");
            continue;
        }
